...
from mcp_bridge.session_registry import SessionRegistryError, registry
...
def _resolve_requested_parameter_value(
    adapter: OspsuiteAdapter,
    payload: SetParameterValueRequest,
) -> tuple[float, str | None, float | None]:
    if payload.update_mode != "relative":
        return payload.value, payload.unit, None

    try:
        current = adapter.get_parameter_value(payload.simulation_id, payload.parameter_path)
    except AdapterError as exc:
        raise SetParameterValueValidationError(
            "Relative parameter updates require an existing parameter value"
        ) from exc

    requested_unit = payload.unit.strip() if payload.unit else None
    current_unit = current.unit.strip() if current.unit else None
    if requested_unit and current_unit and requested_unit != current_unit:
        raise SetParameterValueValidationError(
            "Relative parameter update unit must match the current parameter unit"
        )

    effective_unit = current_unit or requested_unit
    return current.value + payload.value, effective_unit, current.value
...
def set_parameter_value(
    ...
    *,
    ...
    requested_value, requested_unit, old_value = _resolve_requested_parameter_value(
        adapter, payload
    )
    ...
    consistent, violations = consistency_validator.validate(
        payload.parameter_path, requested_value, requested_unit
    )
    if not consistent:
