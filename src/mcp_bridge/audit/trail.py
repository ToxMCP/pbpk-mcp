...
def _canonicalize_for_hash(obj: Any) -> Any:
    ...
    if isinstance(obj, float):
        ...
        if obj == float("-inf"):
            return "-Infinity"
        # Quantize to 14 decimal places for cross-platform determinism
        quantized = Decimal(obj).quantize(
            Decimal("0.00000000000000"), rounding=ROUND_HALF_UP
        )
    ...
    if isinstance(obj, str):
        # Normalize unicode to guard against platform differences
...
def compute_event_hash(event: dict[str, Any]) -> str:
    ...
    canonical = _canonicalize_for_hash(temp)
    payload = json.dumps(canonical, separators=(",", ":"), sort_keys=True, ensure_ascii=True).encode("utf-8")
