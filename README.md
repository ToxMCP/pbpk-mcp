...
- Hardened audit and operator-release workflows with S3-compatible smoke coverage, fast review-signoff indexing plus backfill support, and clearer GitHub `main` ruleset requirements before tagging.
...
python3 scripts/public_release_preflight.py --auth-dev-secret pbpk-local-dev-secret
...
3. Recreate the packaged local stack with `./scripts/deploy_rxode2_stack.sh`.
   ...
   - When you want a packaged local proof of the S3 backend without real AWS infrastructure, use `./scripts/deploy_s3_audit_smoke_stack.sh` and then `python3 scripts/s3_audit_smoke.py --auth-dev-secret pbpk-local-dev-secret`.
